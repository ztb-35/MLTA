...
class TEMPO(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.is_gpt:
            if self.pretrain:
                self.gpt2_trend = GPT2Model.from_pretrained('gpt2', output_attentions=True,
            ...
            self.gpt2_trend_token = self.tokenizer(text="Predict the future time step given the trend",
                                                   return_tensors="pt")
            self.gpt2_season_token = self.tokenizer(text="Predict the future time step given the season",
                                                    return_tensors="pt")
            self.gpt2_residual_token = self.tokenizer(text="Predict the future time step given the residual",
                                                      return_tensors="pt")
        ...
        self.in_layer_season = nn.Linear(configs.patch_len, configs.d_model)
        self.in_layer_noise = nn.Linear(configs.patch_len, configs.d_model)
            ...
            if type == 'Trend':
                if self.pool:
                    ...
                    for selected_prompt_trend in selected_prompts_trend:
                        self.prompt_record_trend[selected_prompt_trend] = self.prompt_record_trend.get(
                            selected_prompt_trend, 0) + 1
                    selected_prompts = selected_prompts_trend
                else:
                    prompt_x = self.gpt2_trend.wte(tokens)
                    prompt_x = prompt_x.repeat(a, 1, 1)
                    prompt_x = self.prompt_layer_trend(prompt_x.to(x.device))
                ...
                if self.pool:
                    ...
                    for selected_prompt_season in selected_prompts_season:
                        self.prompt_record_season[selected_prompt_season] = self.prompt_record_season.get(
                            selected_prompt_season, 0) + 1
                    selected_prompts = selected_prompts_season
                else:
                    prompt_x = self.gpt2_trend.wte(tokens)
                    prompt_x = prompt_x.repeat(a, 1, 1)
                    prompt_x = self.prompt_layer_season(prompt_x.to(x.device))
                ...
                if self.pool:
                    ...
                    for selected_prompt_resid in selected_prompts_resid:
                        self.prompt_record_residual[selected_prompt_resid] = self.prompt_record_residual.get(
                            selected_prompt_resid, 0) + 1
                    selected_prompts = selected_prompts_resid
                else:
                    prompt_x = self.gpt2_trend.wte(tokens)
                    prompt_x = prompt_x.repeat(a, 1, 1)
                    prompt_x = self.prompt_layer_noise(prompt_x.to(x.device))
    ...
    def forward(self, x, batch_x_mark, dec_inp, batch_y_mark, trend, season, noise, test=False):
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                trend, reduce_sim_trend, trend_selected_prompts = self.get_emb(trend,
                                                                               self.gpt2_trend_token['input_ids'].to(device),
                                                                               'Trend')
            else:
                trend = self.get_emb(trend, self.gpt2_trend_token['input_ids'].to(device), 'Trend')
        else:
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                season, reduce_sim_season, season_selected_prompts = self.get_emb(season,
                                                                                  self.gpt2_season_token['input_ids'].to(device),
                                                                                  'Season')
            else:
                season = self.get_emb(season, self.gpt2_season_token['input_ids'].to(device), 'Season')
        else:
        ...
        if self.is_gpt and self.prompt == 1:
            if self.pool:
                noise, reduce_sim_noise, noise_selected_prompts = self.get_emb(noise,
                                                                               self.gpt2_residual_token['input_ids'].to(device),
                                                                               'Residual')
            else:
                noise = self.get_emb(noise, self.gpt2_residual_token['input_ids'].to(device), 'Residual')
        else:
