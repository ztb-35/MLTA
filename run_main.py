...
from torch.optim import lr_scheduler
from tqdm import tqdm
from models import Autoformer, DLinear, TimeLLM, ST_TimeLLM_1, ST_TimeLLM
from data_provider.data_factory import data_provider
import time
import random
import numpy as np
...
for ii in range(args.itr):
    ...
    test_loss, test_mae_loss = vali(args, accelerator, model, test_data, test_loader, criterion, mae_metric)
    accelerator.print("Best model: MSE: {0:.7f} MAE: {1:.7f}".format(test_loss, test_mae_loss))  # loss=MSE,mae_loss=MAE    mses.append(test_loss)
