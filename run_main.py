...
def plot_input(path, sample, saved_file_name):
    ...
    plt.legend()
    plt.grid(True)
    output_path = path+'/'+saved_file_name
...
if not args.output_attn_map:
    for ii in range(args.itr):
        ...
        # Adjust the keys by removing 'module.' prefixelse:
        new_state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
        model.load_state_dict(new_state_dict)
        model = nn.DataParallel(model)
        model.to(device)
        test_loss, test_mae_loss = vali(args, device, model, test_data, test_loader, criterion, mae_metric)
        print("Best model: MSE: {0:.7f} MAE: {1:.7f}".format(test_loss, test_mae_loss))  # loss=MSE,mae_loss=MAE    mses.append(test_loss)
