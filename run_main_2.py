...
from models import Autoformer, DLinear, TimeLLM, ST_TimeLLM, ST_TimeLLM_1, ST_TimeLLM_2, ST_TimeLLM_3
...
from utils.tools import EarlyStopping, adjust_learning_rate, vali, test, del_files, vali_tempo
...
def main():
    ...
    for ii in range(args.itr):
        ...
        elif args.model == 'ST_TimeLLM':
            model = ST_TimeLLM.Model(args).float()
        ...
        accelerator.print("original lr = {:.10f}".format(args.learning_rate))
        accelerator.print("optimizer lr = {:.10f}".format(model_optim.param_groups[0]['lr']))
        ...
        for epoch in range(args.train_epochs):
                ...
                if args.use_amp:
                    with torch.cuda.amp.autocast():
                        if args.output_attention:
                            outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, seq_trend, seq_seasonal, seq_resid)[0]
                        else:
            ...
            vali_loss, vali_mae_loss = vali_tempo(args, accelerator, model, vali_data, vali_loader, criterion,mae_metric)
            test_loss, test_mae_loss = vali_tempo(args, accelerator,model, test_data,test_loader,criterion,mae_metric)
